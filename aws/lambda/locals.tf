locals {
  ...
  routing_config          = "${var.additional_version_weights != null ? tolist([var.additional_version_weights]) : local.empty_array}"
  ...
  ignore_changes          = "${var.publish ? tolist(["source_code_hash", "qualified_arn", "version", "last_modified"]) : null}"
